...
Sprite::Sprite(ID3D11Device* device, const char* filename)
{
	...
	if (filename != nullptr)
	{
		...
		OutputDebugStringA("Loading Sprite");
	}
	else
	{
		// ダミーテクスチャ生成
		...
		OutputDebugStringA("No Loading Sprite nullptr");
	}
}
...
//        float sy = sh * (c >> 4);
//
//        Render(dc, x + carriage, y, 0, w, h, sx, sy, sw, sh, 0, r, g, b, a);
//		carriage += w;
//	}
//}